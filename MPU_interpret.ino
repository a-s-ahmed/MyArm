...
void loop() {
  ...
  accAngleZ = (atan(AccX/ sqrt(pow(AccY, 2) + pow(AccX, 2))) * 180 / PI) + AccErrorZ; 
  ...
  GyroX = round(GyroX +GyroErrorX); // GyroErrorX ~(-0.56)
  GyroY = round(GyroY + GyroErrorY); // GyroErrorY ~(2)
  GyroZ = round(GyroZ + GyroErrorZ); // GyroErrorZ ~ (-0.8)

  
  // Currently the raw values are in degrees per seconds, deg/s, so we need to multiply by sendonds (s) to get the angle in degrees
  gyroAngleX =(gyroAngleX + GyroX * elapsedTime); // deg/s * s = deg
  //Serial.print(gyroAngleX), Serial.print("     |     "), Serial.println(accAngleX);
  //Serial.println("");
