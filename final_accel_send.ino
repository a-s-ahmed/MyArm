...
const int MPU = 0x68; // one one arm, AD0 is LOW

const int MPU2 = 0x69; // one the hand, AD0 is HIGH
...
*/
  ...
  //roll = int(0.90 * gyroAngleX +0.1 * accAngleX)%360;

  //pitch = int(0.90 * gyroAngleY + 0.1 * accAngleY)%360;
  ...
  */
  ...
  int val[4] = {floor(accAngleX - 500), floor(accAngleY+ 500),floor(accAngle2X-1500), floor(accAngle2Y+1500)};

  //Serial.print(accAngleX), Serial.print("     |   "), Serial.println(accAngleY);

  radio.write(&val[0],sizeof(val[0]));

  radio.write(&val[1],sizeof(val[1]));

  radio.write(&val[2],sizeof(val[2]));

  radio.write(&val[3],sizeof(val[3]));
